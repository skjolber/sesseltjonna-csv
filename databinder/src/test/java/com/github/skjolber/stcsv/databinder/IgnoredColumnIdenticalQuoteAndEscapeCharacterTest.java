...
public class IgnoredColumnIdenticalQuoteAndEscapeCharacterTest {
	...
	public void identicalQuoteAndEscapeCharactersSkipNoEscape() throws IOException {
		...
		offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipColumnsWithoutLinebreak(quotedNoEscapeNoLinebreak.toCharArray(), 0, ',', '\'', 2);
		assertThat(offset).isEqualTo(quotedNoEscapeNoLinebreak.lastIndexOf(',') + 1);
		
		offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipToLineBreak(scanner, quotedNoEscapeLinebreak.toCharArray(), 0, '\'');
		assertThat(offset).isEqualTo(quotedNoEscapeLinebreak.length());

		offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipToLineBreakWithoutLinebreak(quotedNoEscapeNoLinebreak.toCharArray(), 0, '\'');
	...
	public void identicalQuoteAndEscapeCharactersSkipEscaped() throws IOException {
		...
		offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipColumnsWithoutLinebreak(quotedEscapeNoLinebreak.toCharArray(), 0, ',', '\'', 2);
		assertThat(offset).isEqualTo(quotedEscapeNoLinebreak.lastIndexOf(',') + 1);
		
		offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipToLineBreak(scanner, quotedEscapeLinebreak.toCharArray(), 0, '\'');
		assertThat(offset).isEqualTo(quotedEscapeLinebreak.length());

		offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipToLineBreakWithoutLinebreak(quotedEscapeNoLinebreak.toCharArray(), 0, '\'');
	...
	public void testFillSkipColumns() throws IOException {
		...
		int offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipColumns(scanner, buffer, 0, ',', '\'', 2);
	...
	public void testFillSkipNewline() throws IOException {
		...
		int offset = IgnoredColumn.IdenticalQuoteAndEscapeCharacter.skipToLineBreak(scanner, buffer, 0, '\'');
